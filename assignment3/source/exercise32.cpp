                ...
                if (current_segment.b == current_event.p) {
                    ...
                    */
                    if (!active_segments.empty()) {
                        ...
                        int right_ind = index;
                        while (right_ind + 1 < active_segments.size() && share_point(current_segment, active_segments[right_ind + 1])) {
                            right_ind += 1;
                        }
...
int main (){
    ...
    ifstream cin ("C:\\Users\\Elke\\Desktop\\Anton\\Realtime-Rendering\\assignment3\\source\\input-2.txt");
    cin >> n;
    while(n--){
        cin >> id >> x0 >> y0 >> x1 >> y1;
        point a{x0,y0,id};
